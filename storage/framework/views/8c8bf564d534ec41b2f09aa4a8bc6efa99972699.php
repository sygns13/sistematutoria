...
<script type="text/javascript">
   $(function () {
                ...
                $("#cabeceraTexto").html('Gestión de Dimensiones Personales <small>Página principal</small>');
    ...
    $(document).ready(function() {
      ...
      $("#btnGuardar").click(function(){
            ...
            var v1=$("#txtnom").val();
            var v2=CKEDITOR.instances['txtdesc'].getData(); 
              ...
              if(v1.length==0){
                  swal({
                    title: '',
                    text: 'Ingrese el Nombre de la Dimensión Personal',
                    type: 'warning',
...
function saveFuncion(v1,v2) {
  
  var token=$("#idToken").val();



  $.post( "dimensions/create", {nom:v1, desc:v2, _token:token}).done(function(data) {
        ...
        $("#formDatos").hide( 'slow', function() {  
                        ...
                        swal({
                      title: 'Registrado',
                      text: 'Se registró la Nueva Dimensión Satisfactoriamente',
                      type: 'success',
                      confirmButtonText: 'Aceptar'
                    });
...
{
$("#boxTitulo").html('DIMENSIÓN PERSONAL: '+nom);


    var token=$("#idToken").val();


  $.post( "dimensions/cargar", {idD:id, _token:token}).done(function(data) {
...
function guardarEditar1(){
    ...
    var v1=$("#txtnomE").val();
    var v2=CKEDITOR.instances['txtdescE'].getData(); 
...
function saveEdit1(v1,v2) {
  ...
  $.post( "dimensions/edit1", {nom:v1, desc:v2, idD:id, _token:token}).done(function(data) {
                ...
                swal({
                      title: 'Editado',
                      text: 'Se modificaron los Datos de la Dimensión seleccionada Satisfactoriamente',
                      type: 'success',
                      confirmButtonText: 'Aceptar'
                    });
...
function eliminar(id, nom)
{
     swal({
  title: '¿Estás seguro?',
  text: "Desea eliminar la dimensión : "+nom+" -- Nota: Solo se podrá eliminar una dimensión que no tenga registro de preguntas asociadas a ella, u otros procesos de Tutoría.",
  type: 'warning',
  showCancelButton: true,
  confirmButtonColor: '#3085d6',
  cancelButtonColor: '#d33',
  confirmButtonText: 'Si, eliminar'
    ...
    $.post( "dimensions/destroy", {idD:id,  _token:token}).done(function(data) {
